...
public class SwiftLoader: UIView {
    ...
    public class func show(animated: Bool) {
        self.show(title: nil, imageName: "", animated: animated)
    }
    
    public class func show(title: String?, imageName: String, animated : Bool) {
        ...
        loader.bgImageView?.image = UIImage(named: imageName)
        loader.bgImageView?.contentMode = .scaleAspectFit
        loader.bgImageView?.backgroundColor = loader.config.centerImageBackgroundColor
    }
    ...
    private func update() {
        ...
        self.titleLabel?.font = self.config.titleTextFont // Font can be change as per our requirement
        ...
        if self.bgImageView == nil {
            bgImageView = UIImageView(frame: CGRect(x: (loadingView?.frame.origin.x ?? 0)+5, y: (loadingView?.frame.origin.y ?? 0)+5, width: (loadingView?.frame.size.width ?? 0)-10, height: (loadingView?.frame.size.height ?? 0)-10))
        }
        else {
            self.bgImageView?.frame = self.frameForImageView()
        }
        bgImageView?.clipsToBounds = true
        bgImageView?.layer.cornerRadius = (bgImageView?.frame.size.height ?? 0)/2
        self.addSubview(bgImageView!)
    }
    
    func frameForImageView() -> CGRect {
        return CGRect(x: (loadingView?.frame.origin.x ?? 0)+5, y: (loadingView?.frame.origin.y ?? 0)+5, width: (loadingView?.frame.size.width ?? 0)-10, height: (loadingView?.frame.size.height ?? 0)-10)
    }
    ...
    public struct Config {
        ...
        public var centerImageBackgroundColor = UIColor.white
...
func showIndicator() {
    ...
    config.spinnerColor = .appColorBlue
    config.titleTextColor = .appColorBlue
    config.spinnerLineWidth = 2.0
    config.foregroundColor = .appColorTextFieldBG
    config.foregroundAlpha = 0.6
    config.centerImageBackgroundColor = .appBlueColor
    
    SwiftLoader.setConfig(config: config)
    // SwiftLoader.show(animated: true)
    SwiftLoader.show(title: "Please wait...", imageName: "ic_splash_logo", animated: false)
}
*/
